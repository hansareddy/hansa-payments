            ...
            <ScrollView contentContainerStyle={styles.modalScroll}>
              ...
              <View style={{ marginTop: 12 }}>
                <Text style={{ fontSize: 13, fontWeight: '700', color: '#0F172A', marginBottom: 8 }}>
                  STB Location Registry ({allCustomers.filter(c => c.latitude && c.longitude).length} Mapped / {allCustomers.length} Accounts)
                </Text>
                {allCustomers.filter(c => c.latitude && c.longitude).length === 0 ? (
                  <View style={{ padding: 16, backgroundColor: '#F8FAFC', borderRadius: 10, alignItems: 'center' }}>
                    <Text style={{ fontSize: 13, color: '#64748B' }}>No customer STB locations logged yet.</Text>
                  </View>
                ) : (
                  allCustomers.filter(c => c.latitude && c.longitude).map(c => (
                    <TouchableOpacity
                      key={c.rowIndex}
                      style={{
                    ...
                    >
                      <View style={{ flex: 1 }}>
                        ...
                        <Text style={{ fontSize: 11, color: '#64748B' }}>
                          {c.boxNo ? `Box #${c.boxNo} • ` : ''}STB: {c.serialNumber || 'No Serial'} • GPS: {parseFloat(c.latitude).toFixed(4)}, {parseFloat(c.longitude).toFixed(4)}
                        </Text>
                      </View>
